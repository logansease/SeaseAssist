...
@implementation SAMBProgressHUD
...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    SAMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    SAMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hide:animated];
        return YES;
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [SAMBProgressHUD allHUDsForView:view];
    for (SAMBProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hide:animated];
    }
    return [huds count];
...
+ (MB_INSTANCETYPE)HUDForView:(UIView *)view {
    ...
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (SAMBProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.animationType = SAMBProgressHUDAnimationFade;
        self.mode = SAMBProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)show:(BOOL)animated {
    NSAssert([NSThread isMainThread], @"SAMBProgressHUD needs to be accessed on the main thread.");
...
- (void)showUsingAnimation:(BOOL)animated {
    ...
    if (animated && animationType == SAMBProgressHUDAnimationZoomIn) {
        self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
    } else if (animated && animationType == SAMBProgressHUDAnimationZoomOut) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == SAMBProgressHUDAnimationZoomIn || animationType == SAMBProgressHUDAnimationZoomOut) {
...
- (void)hideUsingAnimation:(BOOL)animated {
    ...
    if (animated && showStarted) {
        ...
        // in the done method
        if (animationType == SAMBProgressHUDAnimationZoomIn) {
            self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(1.5f, 1.5f));
        } else if (animationType == SAMBProgressHUDAnimationZoomOut) {
            self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
        }
...
- (void)updateIndicators {
    ...
    if (mode == SAMBProgressHUDModeIndeterminate) {
        ...
        [self addSubview:indicator];
    } else if (mode == SAMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        self.indicator = nil;
    }
}
